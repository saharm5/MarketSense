  ...
  constructor(@Inject(PLATFORM_ID) private platformId: object) {
    if (isPlatformBrowser(this.platformId)) {
      import('highcharts/modules/treemap').then(() => {
        ...
        // Get min and max values for color axis dynamically based on data
        ...
        this.chartOptions = {
          ...
          tooltip: {
            ...
            pointFormat:
              '<b style="color: #f73539;">Market Cap:</b> USD {point.value} Bln',
            style: {
              color: '#f1f1f1',
              fontSize: '14px',
            },
          ...
          series: [
            {
              name: 'All',
              colorByPoint: false,  // اجازه بده از colorAxis استفاده کنه
              colorKey: 'colorValue', // از colorValue برای تعیین رنگ استفاده کن
              type: 'treemap',
              ...
              data: this.getLargeDataSet().map(item => ({
                ...item,
                value: item.value,
          ...
          colorAxis: {
            min: minValue,
              max: maxValue,   // یا استفاده از maxValue از داده‌ها
            stops: [
              [0, '#f73539'],     // قرمز (منفی)
              [0.5, '#999999'],   // خاکستری
              [1, '#2ecc59'],     // سبز (مثبت)
